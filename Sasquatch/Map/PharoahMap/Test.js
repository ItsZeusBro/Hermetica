...
import {Utils, Rand, Encoding} from "../../Utils/Utils.js"
...
export class PharoahMapTest{
    ...
    latinMap(){
        ...
        for(var i = 0; i<latinList.length; i++){
            assert.equal(new Encoding().string2HexBE(latinList[i]), latinMap[latinList[i]]['hex'])
        }
    }
    ...
    _translatationMap(map){
        for(var i=0; i<Object.keys(map['variables']).length;i++){
            ...
            assert(new Encoding().string2HexBE(key) == hex, true)
            assert(new Encoding().hex2BytesBE(hex) == bin, true)
            assert(new Encoding().bin2Decimal(bin) == decimal, true)
    ...
    latinList(){
        ...
        for(var i=0; i<latinList.length-1; i++){
            console.log(latinList[i], latinList[i+1])
            console.log(new Encoding().hex2BytesBE(new Encoding().string2HexBE(latinList[i])))
            console.log(new Encoding().hex2BytesBE(new Encoding().string2HexBE(latinList[i+1])))

            var latin1 = parseInt(new Encoding().hex2BytesBE(new Encoding().string2HexBE(latinList[i])), 2)
            var latin2 = parseInt(new Encoding().hex2BytesBE(new Encoding().string2HexBE(latinList[i+1])), 2)

            assert.equal(latin2-latin1, 1)
        }
	}
    regexList(){
        ...
        for(var i=0; i<regexList.length-1; i++){
            var regex1 = parseInt(new Encoding().hex2BytesBE(new Encoding().string2HexBE(regexList[i])), 2)
            var regex2 = parseInt(new Encoding().hex2BytesBE(new Encoding().string2HexBE(regexList[i+1])), 2)
            assert.equal(regex2-regex1, 1)
        }
    }
    cairoList(){
        ...
        for(var i=0; i<cairoList.length-1; i++){
            var cairo1 = parseInt(new Encoding().hex2BytesBE(new Encoding().string2HexBE(cairoList[i])), 2)
            var cairo2 = parseInt(new Encoding().hex2BytesBE(new Encoding().string2HexBE(cairoList[i+1])), 2)
            assert.equal(cairo2-cairo1, 1)
        }
	}
