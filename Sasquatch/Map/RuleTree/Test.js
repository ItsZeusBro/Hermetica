...
class Test{
    ...
    ruleTree(){
        ...
        for(var m=0; m<=dims-1; m++){
            ...
            for(var neighbor_count=maps[m].map['dimension']; neighbor_count<=2*maps[m].map['dimension']; neighbor_count++){
                for(var i=0; i<maps[m].map['ruleTree']['neighborhoods'][neighbor_count].length; i++){
                        var rule = rules.shift()
                        var neighborhood = maps[m].map['ruleTree']['neighborhoods'][neighbor_count][i]
